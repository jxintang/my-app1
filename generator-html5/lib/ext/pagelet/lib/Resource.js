...
class Resource {
   ...
   */
    addScript(code) {
        if (this._usePagelet && !this._isPageletOpen)
            return;
   ...
   */
    require(id) {
        ...
        if (this._loaded.hasOwnProperty(rId))
            return true;
   ...
   */
    ...
    renderATFCss() {
        let html = '';
        if (this._collect.css && this._collect.css.length) {
            this._collect.css.forEach(this.manifest.combo
                ? function(uri) {
                    let content = '';
    ...
    renderBTFCss() {
        ...
        if (html) {
            html += '<script>(function(d,l,h,i,n,e){' + 'l=d.getElementsByTagName("link");' + 'h=d.head||d.getElementsByTagName("head")[0];' + 'for(i=0,n=l.length;i<n;i++){' + 'e=l[i];' + 'if(e.hasAttribute("data-defer"))' + 'h.appendChild(e);' + '}' + '})(document);</script>';
        }
        return html;
    }
    ...
    pageletStart(id) {
        ...
        if (ret) {
            ...
            this._pageletsStack.push({id: id, fullId: fullId, state: ret});
        }
        return ret;
    }
