...
let Canvas = require('canvas'),
    ...
    defaults = {
        charPool: ('abcdefghijklmnopqrstuvwxyz' +
            'abcdefghijklmnopqrstuvwxyz'.toUpperCase() + '1234567890').split(''),
        size: {
            width: 100,
            height: 32
        },
...
module.exports = function(_opts) {
    ...
    return async(context, next) => {
        if (context.request.path !== defs.url)
            return await next();
        ...
        context.type = 'jpg';
        context.set({'Cache-Control': 'no-cache, private, no-store, must-revalidate, max-stale=0, post-check=0, pre-check=0', 'Expires': 'Sun, 12 Jan 1986 12:00:00 GMT'});

        context.captcha = textobj.code;
        let {query} = context.request;
        if (query.captcha) {
            context.cookies.set(query.captcha, sha1(context.captcha.toLowerCase()), {
                signed: true,
                httpOnly: false
            })
        }
